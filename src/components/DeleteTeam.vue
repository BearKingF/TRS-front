...
//导入接口
import deleteTeamAPI from "../apis/deleteTeamAPI/deleteteam";
//导入资源
...
//声明表单
const id=ref("");
const form = reactive({ member_id:parseInt(id.value)});
const onSubmit = async () => {
    if (form.member_id!=0) {
        try {
            const result = await deleteTeamAPI(form);
            const {  code, msg } = result;
            if (code === 200 && msg === "ok") {
                    ElNotification({ title: "移除成功!"});
                }
            ...
            ElNotification(
                {
                    title: "抱歉",
                    message: h("i", { style: "color:teal" }, `移除失败, ${e.message || "未知错误"}`)
                }
            );
        }
    }
        ...
        ElNotification({
            title: "失败",
            message: h("i", { style: "color: teal" }, "移除失败，请将必填内容输入完整!"),
        });
    };
};
const clear = () => {
...
<template>
        ...
        <el-form :model="form" label-width="120px">
            <el-form-item label="*移除成员id">
                <el-input v-model="id" />
            </el-form-item>
            <el-form-item>
                <el-button type="primary" @click="onSubmit" class="button">移除</el-button>
