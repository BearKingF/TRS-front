...
//导入接口
import changeCapAPI from "../apis/changeCapAPI/changecap";
//导入资源
...
const id = ref("");
const form = reactive({
        member_id: parseInt(id.value)
    }
);
const onSubmit = async () => {
    if (form.member_id !==0) {
        try {
            const result = await changeCapAPI(form);
            const { code, msg } = result;
            if (code === 200 && msg === "ok") {
                ElNotification({ title: "转移成功!" });
                router.push("/user");
            }
            ...
            ElNotification(
                {
                    title: "抱歉",
                    message: h("i", { style: "color:teal" }, `转移失败, ${e.message || "未知错误"}`)
                }
            );
        }
    }
        ...
        ElNotification({
            title: "失败",
            message: h("i", { style: "color: teal" }, "转移失败，请将必填内容输入完整!"),
        });
    };
};
const clear = () => {
...
<template>
        ...
        <el-form :model="form" label-width="120px">
            <el-form-item label="*要转给的成员id">
                <el-input v-model="id" />
            </el-form-item>
            <el-form-item>
                <el-button type="primary" @click="onSubmit" class="button">转移队长职位</el-button>
