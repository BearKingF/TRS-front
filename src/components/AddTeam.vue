...
//导入接口
import addTeamAPI from "../apis/addTeamAPI/addteam";
//导入资源
...
const onSubmit = async () => {
    if (form.member_id!== 0) {
        try {
            const result = await addTeamAPI(form);
            const {  code, msg } = result;
            if (code === 200 && msg === "ok") {
                    ElNotification({ title: "添加成功!"});
                }
            ...
            ElNotification(
                {
                    title: "抱歉",
                    message: h("i", { style: "color:teal" }, `添加失败, ${e.message || "未知错误"}`)
                }
            );
        }
    }
        ...
        ElNotification({
            title: "失败",
            message: h("i", { style: "color: teal" }, "添加失败，请将必填内容输入完整!"),
        });
    };
};
const clear = () => {
...
<template>
        ...
        <el-form :model="form" label-width="120px">
            <el-form-item label="*要添加团队成员id">
                <el-input v-model="id" />
            </el-form-item>
            <el-form-item>
                <el-button type="primary" @click="onSubmit" class="button">添加</el-button>
